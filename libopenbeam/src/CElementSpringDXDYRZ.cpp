...
#include <openbeam/CElementSpringDXDYRZ.h>
#include <openbeam/CStructureProblem.h>

using namespace std;
using namespace openbeam;

CElementSpringDXDYRZ::CElementSpringDXDYRZ() : CElement(2), Kx(0), Ky(0) {}

CElementSpringDXDYRZ::CElementSpringDXDYRZ(
    const size_t from_node_id, const size_t to_node_id, const num_t Kx,
    const num_t Ky, const num_t KRz)
    : CElement(2, from_node_id, to_node_id), Kx(Kx), Ky(Ky), KRz(KRz)
{
}
 ...
 */
void CElementSpringDXDYRZ::getLocalStiffnessMatrices(
...
void CElementSpringDXDYRZ::getLocalDoFs(std::vector<used_DoFs_t>& dofs) const
{
    dofs.resize(getNumberEdges());

    used_DoFs_t sDofs = {Kx != 0, Ky != 0, false, false, false, KRz != 0};
    dofs[0] = dofs[1] = sDofs;
}

/** Parse a set of parameters by (casi insensitive) name and set the element
 * values from them. */
void CElementSpringDXDYRZ::loadParamsFromSet(
...
void CElementSpringDXDYRZ::do_mesh(
