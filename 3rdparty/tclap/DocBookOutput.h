...
inline void DocBookOutput::usage(CmdLineInterface& _cmd)
{
    ...
    for (ArgListIterator it = argList.begin(); it != argList.end(); it++)
        if (!xorHandler.contains((*it))) printShortArg((*it));
    ...
    // rest of args
    for (ArgListIterator it = argList.begin(); it != argList.end(); it++)
        if (!xorHandler.contains((*it))) printLongArg((*it));
...
{
    size_t p;
    while ((p = s.find_first_of(r)) != std::string::npos) { s.erase(p, 1); }
}
...
inline void DocBookOutput::printShortArg(Arg* a)
{
    ...
    if (a->isRequired()) choice = "req";

    std::string repeat = "norepeat";
    if (a->acceptsMultipleValues()) repeat = "repeat";

    std::cout << "<arg choice='" << choice << "' repeat='" << repeat << "'>"
              << id << "</arg>" << std::endl;
}
