...
template <class MATRIX, class ARRAY> void print_html_matrix(const MATRIX &K,const vector<string> &row_titles, const vector<string> &col_titles, const ARRAY * rows_to_use = NULL,const ARRAY * cols_to_use = NULL );
template <class MATRIX, class ARRAY> void print_csv_matrix(const std::string &out_csv_file, const MATRIX &K,const vector<string> &row_titles, const vector<string> &col_titles, const ARRAY * rows_to_use = NULL,const ARRAY * cols_to_use = NULL );

void usedDOFs2titles(size_t node_id, vector<string> &titles, bool html, const TUsedDoFs * edge_dofs = NULL);
void listDOFs2titles(const std::vector<TDoF> & dofs,vector<string> &dof_titles, bool html);
...
int main_code(int argc, char**argv)
{
	...
	TCLAP::ValueArg<std::string> arg_problemFile("i","input","Problem file to load",true,"","problem.txt",cmd);
	TCLAP::ValueArg<int>         arg_verbose_level("v","verbose","Verbosity level (0-3)",false,1,"0-3",cmd);
	TCLAP::ValueArg<std::string> arg_language("l","lang","Language (ISO 639-1 code)",false,"en","en,es,...",cmd);
	TCLAP::ValueArg<std::string> arg_output("o","output","Switch (text or html) output to a target file",false,"","output.html",cmd);

	TCLAP::SwitchArg             arg_meshing("","mesh","Enable meshing (Default: no)",cmd);
	TCLAP::ValueArg<double>      arg_mesh_resolution("","mesh-resolution","Meshing resolution (Default: 0.05 m)",false,0.05,"0.05",cmd);

	TCLAP::SwitchArg             arg_html("","html","Enable HTML output (Default: no)",cmd);
	...
	TCLAP::ValueArg<std::string> arg_plots_width("","plots-width","Stress and deformation plot sizes for HTML output",false,"1000px","1000px",cmd);
	TCLAP::ValueArg<std::string> arg_plots_height("","plots-height","Stress and deformation plot sizes for HTML output",false,"300px","300px",cmd);

	TCLAP::SwitchArg             arg_show_FL("","show-FL","Text display of the vector of forces in free nodes (Default: no)",cmd);
	TCLAP::SwitchArg             arg_show_FR("","show-FR","Text display of reactions (Default: no)",cmd);
	TCLAP::SwitchArg             arg_show_U("","show-U","Text display of all displecements (Default: no)",cmd);

	TCLAP::SwitchArg             arg_full_K("","full-k","Enable displaying the full stiffness matrix (K) for the entire problem (Default: no)",cmd);
	TCLAP::ValueArg<std::string> arg_full_K_csv("","full-k-csv","Dumps full K matrix to a CSV file",false,"","full-k.csv",cmd);
	TCLAP::SwitchArg             arg_all_elements_K("","all-element-k","Enable displaying the global stiffness matrices for every element (Default: no)",cmd);

	TCLAP::ValueArg<std::string> arg_out_animation("","animate-deformed","Creates an animation of the structure deformed state and saves to target GIF file",false,"","anim-deformed.gif",cmd);
	...
	const bool calc_stress = arg_stress.getValue() || arg_stress_plots.getValue();
	...
	if (arg_output.isSet())
	{
		// By creating this object all will be handled automatically: We'll just need to write to cout as normal.
		const std::string fil_redirect_output = arg_output.getValue();
		console_redirect = auto_ptr<CConsoleRedirector>(new CConsoleRedirector(fil_redirect_output,false,false));
	}
	...
	if (out_html && out_svg)
	{
		...
		cout <<
			...
			"<img src=\"" << sFilOriginal << "\" width=\"" << arg_out_images_width.getValue() <<  "\"/><br/>\n"
		...
		cout <<
			...
			"<img src=\"" << sFilDeformed << "\" width=\"" << arg_out_images_width.getValue() << "\"/><br/>\n"
			<< "("<< _t(STR_click_to_enlarge) << ")</a>"
			"</div><br/>\n";
	...
	if (out_full_k || out_full_k_csv)
	{
        ...
        if (out_full_k)
        {
            ...
            else
            {
                ...
                cout << "<h3>"<<_t(STR_GlobalStiffnessMatrix) << "(K):</h3>\n";
                print_html_matrix<TDynMatrix,TUsedDoFs>(K_full_dense,dof_titles,dof_titles);
            }
        }
        ...
        if (out_full_k_csv)
        {
            ...
            print_csv_matrix<TDynMatrix,TUsedDoFs>(out_full_k_csv_filename, K_full_dense,dof_titles,dof_titles);
        }
	...
	if (out_all_elements_k)
	{
        ...
        if (out_html)
                cout << "<h3>"<<_t(STR_AllElementsStiffnessMatrices) << "(K):</h3>\n";
        else    cout << _t(STR_AllElementsStiffnessMatrices) << ":\n";
	    ...
	    for (size_t i=0;i<nEle;i++)
	    {
	        ...
	        for (size_t i=0;i<subMats.size();i++)
	        {
	            ...
	            usedDOFs2titles(node_in, col_titles, out_html  );  // generate all captions, they'll be
	            usedDOFs2titles(node_out, row_titles, out_html );  // filtered in print_html_matrix()

                if (out_html) {
                    cout << format("K<sub>%u,%u</sub> = ", static_cast<unsigned int>(node_in), static_cast<unsigned int>(node_out) );
                    print_html_matrix<TMatrix66,TUsedDoFs>(sm.matrix,row_titles,col_titles,&edge_dofs_out, &edge_dofs_in);
                }
                else    cout << sm.matrix << endl << endl;
	        }
	    }
	...
	if (out_html)
	{
		if (arg_show_FL.isSet())
		{
			...
			cout << "<tr><td bgcolor=\"#E0E0E0\">"<< _t(STR_dof) << "</td> <td bgcolor=\"#E0E0E0\">(N, Nm)</td></tr>\n";
			...
			for (size_t i=0;i<nF;i++)
			{
				...
				switch(dof.dof)
				{
				case 0: cout << "F<sub>x"; break;
				case 1: cout << "F<sub>y"; break;
				case 2: cout << "F<sub>z"; break;
				case 3: cout << "M<sub>x"; break;
				case 4: cout << "M<sub>y"; break;
				case 5: cout << "M<sub>z"; break;
				};
				cout << dof.node_id;
				cout << "</sub>";
				cout << "</td><td>";
	...
	if (arg_show_U.isSet())
	{
		if (out_html)
		{
			cout << "<h3>" << _t(STR_Displacements) << " (U<sub>R</sub> "<< _t(STR_and) << " U<sub>L</sub>):</h3>\n";
...
//	if (out_html) cout << "</pre>\n";
	...
	if (arg_stress.isSet())
	{
		...
		{
			cout << "\n\nELEMENT STRESS:\n\n";
			cout << "Element|Face|     N    |     Vy   |     Vz   |     Mx   |     My   |     Mz   |\n";
			cout << "-------+----+----------+----------+----------+----------+----------+----------+\n";
		}
		else
		{
			...
			cout << "<table border=\"1\" cellspacing=\"0\" cellpadding=\"3\">\n"
				"<tr><td>Element</td><td>Face</td><td>N</td><td>Vy</td><td>Vz</td><td>Mx</td><td>My</td><td>Mz</td></tr>\n";
		}

		const unsigned int nE = static_cast<unsigned int>( stressInfo.element_stress.size() );
		for (unsigned int i=0;i<nE;i++)
		{
			for (unsigned int face=0;face<stressInfo.element_stress[i].size();face++)
			{
				const TFaceStress & es = stressInfo.element_stress[i][face];
				if (!out_html)
				{
					cout << format ("%s| %2u |%10.3e|%10.3e|%10.3e|%10.3e|%10.3e|%10.3e|\n",
	...
	if (arg_stress_plots.isSet() && out_html && mesh_info!=NULL)
	{
		...
		// Plot sizes:
		const string sGraphStyle = "style='width: " + arg_plots_width.getValue() + "; height: " + arg_plots_height.getValue() + ";'";
		...
		for (size_t idx_bar = 0 ; idx_bar < mi.element2elements.size();idx_bar++)
		{
			...
			const double org_bar_length = problem_to_solve->getNodePose(node0).distanceTo( problem_to_solve->getNodePose(node1) );

			const std::vector<size_t> &idxs = mi.element2elements[idx_bar]; // indices of all the elements that build this bar
			const std::vector<size_t> &idxs_nodes = mi.element2nodes[idx_bar]; // indices of all the nodes that build this bar
			...
			idx_of_first_plot_for_each_beam.push_back(idx_N);  // Associate: idx_bar <==> idx_N
			...
			for (size_t k=0;k<=nE;k++)
			{
				...
				OBASSERT(stressInfo.element_stress[idx_el].size() == 2) // We only have linear elements only yet!

				unsigned int face= k<nE ? 0 : 1;

				// Stress:
				const TFaceStress & es = stressInfo.element_stress[idx_el][face];
				gd_N.x[k] = x;   gd_N.y[k] = 1e-3* es.N;
				gd_Vy.x[k] = x;  gd_Vy.y[k] = -1e-3* es.Vy;  // WARNING: The "-" is to match the sign criterion as explained in our courses
				gd_Mz.x[k] = x;  gd_Mz.y[k] = 1e-3* es.Mz;
				gd_Mx.x[k] = x;  gd_Mx.y[k] = 1e-3* es.Mx;

				// Displacement field:
				const size_t idx_node = idxs_nodes[k];

				gd_Ux.x[k] = x;   gd_Ux.y[k] = 1e3* U[idx_node][0];
				gd_Uy.x[k] = x;   gd_Uy.y[k] = 1e3* U[idx_node][1];
				gd_Rotz.x[k] = x;   gd_Rotz.y[k] = (180/M_PI) * U[idx_node][5];
				gd_Rotx.x[k] = x;   gd_Rotx.y[k] = (180/M_PI) * U[idx_node][3];
		...
		if (arg_plots_continuous_beam.isSet())
		{
			...
			for (size_t i=0;i<sLstContBeamIDs.size();i++)
			{
				...
				if (beam_id>=idx_of_first_plot_for_each_beam.size()) throw std::runtime_error( format("Index of continuous beam out of range: '%s'",sContBeamsIDs.c_str()) );
				...
				gd_Vy.y.insert(gd_Vy.y.end(), gd_Vy_i.y.begin(), gd_Vy_i.y.end() );
				gd_Mz.y.insert(gd_Mz.y.end(), gd_Mz_i.y.begin(), gd_Mz_i.y.end() );
				gd_Ux.y.insert(gd_Ux.y.end(), gd_Ux_i.y.begin(), gd_Ux_i.y.end() );
				gd_Uy.y.insert(gd_Uy.y.end(), gd_Uy_i.y.begin(), gd_Uy_i.y.end() );
				gd_Rotz.y.insert(gd_Rotz.y.end(), gd_Rotz_i.y.begin(), gd_Rotz_i.y.end() );
				gd_Mx.y.insert(gd_Mx.y.end(), gd_Mx_i.y.begin(), gd_Mx_i.y.end() );
				gd_Rotx.y.insert(gd_Rotx.y.end(), gd_Rotx_i.y.begin(), gd_Rotx_i.y.end() );
			}
	...
	if (out_html)
		cout << "</div> <!-- END OF NON GRAPHICAL RESULTS -->\n";
	...
	if (out_html)
	{
		...
		for (size_t i=0;i<lst_html_graphs.size();i++)
		{
			...
			cout <<
			"    var data_"<<i<<" = google.visualization.arrayToDataTable( sResult_"<<i<<" );\n"
			"    var props_"<<i<<" = {legend: { position: 'none'},\n"
			"		vAxis :  {title: '" << gd.y_label <<  "', titleTextStyle: { }, gridlines : {count : 5 } },\n"
			"		hAxis :  {title: '" << gd.x_label <<  "', titleTextStyle: { }, gridlines : {count : 4 } },\n"
			"		pointSize : 2,\n"
			"		title : '"<< gd.title <<"',\n"
			"		};\n"						
			"    var obj_Graph_"<<i<<" = new google.visualization.LineChart(document.getElementById('graph_"<<i<<"')).draw(data_"<<i<<", props_"<<i<<" );\n";
		}
	...
	if (out_html && !out_html_no_head)
	{
		...
		const string html_tail = openbeam::format(
			"\n<br><br><hr><small>Page generated by OpenBeam - Jose Luis Blanco Claraco (C) 2016</small><br>"
			"<div align=\"right\">"
...
void print_html_matrix(
	const MATRIX &K,
	const vector<string> &row_titles,
	const vector<string> &col_titles,
	const ARRAY * rows_to_use,
	...
	)
{
    ...
    for (size_t i=0;i<nRows;i++)
    {
        ...
        for (size_t j=0;j<nCols;j++)
        {
            ...
            if (Kijabs>0)
            {
                const int mag = static_cast<int>( log10(Kijabs) );  // Floor to "int"
                pow10hits[mag]++;
            }
    ...
    if (!pow10hits.empty())
    {
        ...
        for (map<int,int>::const_iterator it=pow10hits.begin();it!=pow10hits.end();++it)
        {
    ...
    const num_t common_factor     = pow(static_cast<num_t>(10),static_cast<num_t>(common_factor_pow));
	...
	)
{
    ...
    if (!f.is_open())
        throw std::runtime_error("print_csv_matrix: Error opening output file.");
...
void usedDOFs2titles(size_t node_id, vector<string> &titles, bool html, const TUsedDoFs * edge_dofs )
{
    for (int k=0;k<6;k++)
    {
...
void listDOFs2titles(const std::vector<TDoF> & dofs,vector<string> &dof_titles, bool html)
{
    ...
    for (size_t i=0;i<nTot;i++)
    {
        ...
        if (html)
                s = format("<td bgcolor=\"#E0E0E0\"> %u (", static_cast<unsigned int>(i) );
        else    s = format("%u (", static_cast<unsigned int>(i) );
        ...
        if (html)
                s+= format("<sub>%u</sub>)</td> ", static_cast<unsigned int>(dof.node_id));
        else    s+= format("_%u)", static_cast<unsigned int>(dof.node_id));
    }
}
