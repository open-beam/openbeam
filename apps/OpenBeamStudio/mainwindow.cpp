...
void MainWindow::on_tvBrowserDirs_clicked(const QModelIndex& index)
{
    ...
    if (is_file && !sPath.isEmpty()) this->loadOpenBeamFile(sPath);
}

void MainWindow::on_action_Open_model_triggered()
{
    QString fileName = QFileDialog::getOpenFileName(
        this, tr("Load model file"),
        m_app_setting.value("app/default_dir", ".").toString(),  // Default path
        tr("OpenBeam files (*.ob *.txt *.TXT *.OB)"));
    if (!fileName.isEmpty()) this->loadOpenBeamFile(fileName);
}
...
void MainWindow::loadOpenBeamFile(const QString& sPath)
{
    ...
    if (file.open(QFile::ReadOnly | QFile::Text))
    {
        ...
        m_app_setting.setValue(
            "app/default_dir", QFileInfo(sPath).absolutePath());
    }
    else
    {
        ...
        QMessageBox::warning(
            this, tr("OpenBeamStudio"), tr("Cannot open the file!"),
            QMessageBox::Ok);
    }
}
