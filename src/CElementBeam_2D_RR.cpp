...
/** Return the stiffness submatrices between each pair of edges in this element, for the current element state.
  */
void CElementBeam_2D_RR::getLocalStiffnessMatrices( openbeam::aligned_containers<TStiffnessSubmatrix>::vector_t &outSubMats ) const
{
	outSubMats.resize(3);
	...
	OBASSERT(L>0)
	if (E==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RR: Uninitialized parameter E (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
	if (A==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RR: Uninitialized parameter A (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
	if (Iz==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RR: Uninitialized parameter Iz (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));

	if (G==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RR: Uninitialized parameter G (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
	if (J==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RR: Uninitialized parameter J (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
...
void CElementBeam_2D_RR::getLocalDoFs(std::vector<TUsedDoFs> &dofs) const
{
