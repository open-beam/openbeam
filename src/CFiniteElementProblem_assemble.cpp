...
void CFiniteElementProblem::assembleProblem( TBuildProblemInfo & out_info )
{
	...
	map<size_t,size_t>  problem_dof2bounded_dof_indices; // Index in "m_problem_DoFs" to index in "bounded_dof_indices"
	map<size_t,size_t>  problem_dof2free_dof_indices;    // Index in "m_problem_DoFs" to index in "free_dof_indices"
	...
	for (size_t k=0;k<nDOFs;k++)
	{
		...
		{	// Free DOF:
			...
			problem_dof2free_dof_indices.insert(problem_dof2free_dof_indices.end(), std::make_pair(k,new_idx) );
		...
		{	// Bounded DOF:
			...
			problem_dof2bounded_dof_indices.insert(problem_dof2bounded_dof_indices.end(), std::make_pair(k,new_idx) );
    ...
    OB_MESSAGE(2) << nDOFs << " DOFs in the problem: " << free_dof_indices.size() << " free, " << bounded_dof_indices.size() << " constrained.\n";
    OB_MESSAGE(2) << "List of all DOFs:\n"  << getProblemDoFsDescription();

	// Start with a dynamic-sparse matrix, later on we'll convert it into real sparse:
	typedef SparseMatrix<num_t>::Index  index_t;
	const size_t estimated_number_of_non_zero = nNodes*6*6*(1+1);  // Will store only the UPPER triangular part of K.
	...
	for (size_t e=0;e<nElements;e++)
	{
		...
		for (size_t j=0;j<mats.size();j++)
		{
            ...
            const TMatrix66 *K_element=NULL; // Will point to either K_element_org or K_element_nodal
            ...
            {
                // Use a shortcut for the most common case where the local coordinates coincide with global ones:
            ...
            else
            {
                ...
                if (!rt_i.r.isIdentity()) {
                    K_element_nodal.block<3,3>(0,0) = rt_i.r.getRot().transpose() * K_element_nodal.block<3,3>(0,0);
                    K_element_nodal.block<3,3>(0,3) = rt_i.r.getRot().transpose() * K_element_nodal.block<3,3>(0,3);
                    K_element_nodal.block<3,3>(3,0) = rt_i.r.getRot().transpose() * K_element_nodal.block<3,3>(3,0);
                    K_element_nodal.block<3,3>(3,3) = rt_i.r.getRot().transpose() * K_element_nodal.block<3,3>(3,3);
                }
                if (!rt_j.r.isIdentity())  {
                    K_element_nodal.block<3,3>(0,0) = K_element_nodal.block<3,3>(0,0) * rt_j.r.getRot();
                    K_element_nodal.block<3,3>(0,3) = K_element_nodal.block<3,3>(0,3) * rt_j.r.getRot();
                    K_element_nodal.block<3,3>(3,0) = K_element_nodal.block<3,3>(3,0) * rt_j.r.getRot();
                    K_element_nodal.block<3,3>(3,3) = K_element_nodal.block<3,3>(3,3) * rt_j.r.getRot();
                }
            }

			// If  i_node_idx == i_node_idx: Sum to the diagonal block matrix.
			//                    Otherwise: Insert into the matrix (we know for sure it's new).

			// We need to know which DoFs from all the 6 are really used in the node.
			...
			for (int r=0;r<6;r++)
			{
				...
				for (int c= (i_node_idx==j_node_idx ? r : 0) ; // Diagonal blocks: c=[r,5] -> Only upper half
				         c<6;c++)
				{
					...
					else  K_tri.push_back( Eigen::Triplet<double>( r_idx_in_K, c_idx_in_K, K_element->coeff(r,c) ) );
				}
	...
	for (index_t r=0;r<nDOFs_b;r++)
		for (index_t c=r;c<nDOFs_b;c++)
			K_bb_aux.insert(r,c) = K.coeff(static_cast<index_t>(bounded_dof_indices[r]),static_cast<index_t>(bounded_dof_indices[c]));

	// Symmetric K_ff
	for (index_t  r=0;r<nDOFs_f;r++)
		for (index_t  c=r;c<nDOFs_f;c++)
			K_ff_aux.insert(r,c) = K.coeff(static_cast<index_t>(free_dof_indices[r]),static_cast<index_t>(free_dof_indices[c]));
	...
	for (index_t r=0;r<nDOFs_b;r++)
		for (index_t c=0;c<nDOFs_f;c++)
		{
			...
			K_bf_aux.insert(r,c) = (i_f>=i_b) ? K.coeff(i_b,i_f) : K.coeff(i_f,i_b); // Only the UPPER part of K is populated.
		}
	...
	// Before building the list of forces, give children classes the opportunity of
	...
	// -------------------------
	{
		...
		for (size_t i=0;i<nDOFs_f;i++)
		{
			...
			{
				TListOfLoads::const_iterator it = m_loads_at_each_dof.find(dofIdx);
				if (it != m_loads_at_each_dof.end() )
					out_info.F_f[i] = it->second;
			}
			{
				TListOfLoads::const_iterator it = m_loads_at_each_dof_equivs.find(dofIdx);
	...
	for (size_t i=0;i<nNodes;i++)
	{
		...
		for (int k=0;k<6;k++)
		{
			if (dof_i.dof_index[k]!=-1) {
				...
				map<size_t,size_t>::const_iterator it=problem_dof2free_dof_indices.find( dof_i.dof_index[k] );
				...
				{
					// It must be bounded:
					map<size_t,size_t>::const_iterator it2=problem_dof2bounded_dof_indices.find( dof_i.dof_index[k] );
