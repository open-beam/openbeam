...
CElementBeam_2D_RA::CElementBeam_2D_RA() : CBaseElementBeam(false, true) {}
 ...
 */
void CElementBeam_2D_RA::getLocalStiffnessMatrices(
    ...
    const
{
    ...
    if (E == UNINITIALIZED_VALUE)
        throw std::runtime_error(format(
            "CElementBeam_2D_RA: Uninitialized parameter E (Element in %u->%u)",
    ...
    if (A == UNINITIALIZED_VALUE)
        throw std::runtime_error(format(
            "CElementBeam_2D_RA: Uninitialized parameter A (Element in %u->%u)",
            static_cast<unsigned int>(this->conected_nodes_ids[0]),
            static_cast<unsigned int>(this->conected_nodes_ids[1])));
    if (Iz == UNINITIALIZED_VALUE)
        throw std::runtime_error(format(
            "CElementBeam_2D_RA: Uninitialized parameter Iz (Element in "
            "%u->%u)",
            static_cast<unsigned int>(this->conected_nodes_ids[0]),
            static_cast<unsigned int>(this->conected_nodes_ids[1])));
    ...
    K11.matrix(1, 5) = K11.matrix(5, 1) = 3 * E * Iz / L2;
