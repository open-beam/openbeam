...
CElementBeam_2D_RA::CElementBeam_2D_RA() :
...
/** Return the stiffness submatrices between each pair of edges in this element, for the current element state.
  */
void CElementBeam_2D_RA::getLocalStiffnessMatrices( openbeam::aligned_containers<TStiffnessSubmatrix>::vector_t &outSubMats ) const
{
	outSubMats.resize(3);
	...
	OBASSERT(L>0)
	if (E==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RA: Uninitialized parameter E (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
	if (A==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RA: Uninitialized parameter A (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
	if (Iz==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_RA: Uninitialized parameter Iz (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
	...
	K11.matrix(1,5) =
	K11.matrix(5,1) =  3*E*Iz/L2;
	K11.matrix(5,5) =  3*E*Iz/L;
	...
	K12.matrix(5,1) = -K11.matrix(1,5);

}

void CElementBeam_2D_RA::getLocalDoFs(std::vector<TUsedDoFs> &dofs) const
{
    dofs.resize(size_t(getNumberEdges()));

	static const TUsedDoFs sDofsA3 = {true, true, false, false, false, false};
