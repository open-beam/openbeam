...
namespace openbeam
{
	namespace internal
	{
			...
			if (node_pose.r.isIdentity())
			{
				...
				for (size_t k=0;k<nPts;k++)
					...
					if (scale==1)
						...
						for (int l=0;l<3;l++)
							seq_points_global[k].coords[l] = node_pose.t.coords[l] + scale*seq_points_global[k].coords[l];
			}
			else
			{
				...
				for (size_t k=0;k<nPts;k++)
				{
					// POINT_GLOBAL = NODE_GLOBAL + MATRIX33 * POINT_LOCAL
					for (int l=0;l<3;l++)
						seq_points_global[k].coords[l] = node_pose.t.coords[l] + scale*(
		...
		/** */
		void computeOrientationFromTwoPoints(const TPoint3D &t1,const TPoint3D &t2,const num_t rotation_around_x, TRotation3D &R);
