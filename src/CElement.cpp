...
void CElement::getGlobalStiffnessMatrices( openbeam::aligned_containers<TStiffnessSubmatrix>::vector_t  &outSubMats ) const
{
	// Get local matrices:
	// ======================================================
	...
	// Rotate them
	// ======================================================
	const TMatrix33 &R  = m_global_orientation.getRot();  // Reference to the 3D rotation matrix (3x3)
	...
	for (size_t i=0;i<outSubMats.size();i++)
	{
		...
		if ( (M.block(0,0,3,3).array()!=num_t(0)).any() ) M.block(0,0,3,3) =  R * M.block(0,0,3,3) * R.transpose();
		if ( (M.block(0,3,3,3).array()!=num_t(0)).any() ) M.block(0,3,3,3) =  R * M.block(0,3,3,3) * R.transpose();
		if ( (M.block(3,0,3,3).array()!=num_t(0)).any() ) M.block(3,0,3,3) =  R * M.block(3,0,3,3) * R.transpose();
		if ( (M.block(3,3,3,3).array()!=num_t(0)).any() ) M.block(3,3,3,3) =  R * M.block(3,3,3,3) * R.transpose();
	}
}
...
TRotation3D::TRotation3D(const num_t roll,const num_t pitch,const num_t yaw)
{
    ...
    else
    {
        ...
        rot(0,0)= cy*cp;  rot(0,1)= cy*sp*sr-sy*cr;  rot(0,2)= cy*sp*cr+sy*sr;
        rot(1,0)= sy*cp;  rot(1,1)= sy*sp*sr+cy*cr;  rot(1,2)= sy*sp*cr-cy*sr;
        rot(2,0)= -sp;    rot(2,1)= cp*sr;           rot(2,2)= cp*cr;
    }
}

void TRotation3D::matrix2angles(const TMatrix33 &R, num_t &roll,num_t &pitch,num_t &yaw)
{
	OBASSERT_DEBUG( std::abs(sqrt(square(R(0,0))+square(R(1,0))+square(R(2,0))) - 1 ) < 3e-3 )
	OBASSERT_DEBUG( std::abs(sqrt(square(R(0,1))+square(R(1,1))+square(R(2,1))) - 1 ) < 3e-3 )
	OBASSERT_DEBUG( std::abs(sqrt(square(R(0,2))+square(R(1,2))+square(R(2,2))) - 1 ) < 3e-3 )
...
bool TRotation3D::isIdentity() const
{
    ...
    {
        return rot.coeff(0,0)==1 && rot.coeff(1,1)==1 && rot.coeff(2,2)==1;
    }
}
...
void CElement::updateOrientationFromNodePositions()
{
	...
	internal::computeOrientationFromTwoPoints(p1.t,p2.t,m_design_rotation_around_linear_axis,m_global_orientation);
}
...
void CElement::getGlobalDoFs(std::vector<TUsedDoFs> &dofs) const
{
	...
	// Binarize R (this is to prevent numeric innacuracies to make us consider more DoFs than the actual ones):
	bool  RnonZero[3][3];
	...
	for (size_t i=0;i<N;i++)
	{
		...
		{
			// X Y Z
			dofs[i][0] = (local_dofs[i][0] && RnonZero[0][0]) || (local_dofs[i][1] && RnonZero[0][1]) || (local_dofs[i][2] && RnonZero[0][2]);
			dofs[i][1] = (local_dofs[i][0] && RnonZero[1][0]) || (local_dofs[i][1] && RnonZero[1][1]) || (local_dofs[i][2] && RnonZero[1][2]);
			dofs[i][2] = (local_dofs[i][0] && RnonZero[2][0]) || (local_dofs[i][1] && RnonZero[2][1]) || (local_dofs[i][2] && RnonZero[2][2]);
		}
		...
		{
			// thX thY thZ
			dofs[i][3] = (local_dofs[i][3] && RnonZero[0][0]) || (local_dofs[i][4] && RnonZero[0][1]) || (local_dofs[i][5] && RnonZero[0][2]);
			dofs[i][4] = (local_dofs[i][3] && RnonZero[1][0]) || (local_dofs[i][4] && RnonZero[1][1]) || (local_dofs[i][5] && RnonZero[1][2]);
			dofs[i][5] = (local_dofs[i][3] && RnonZero[2][0]) || (local_dofs[i][4] && RnonZero[2][1]) || (local_dofs[i][5] && RnonZero[2][2]);
		}
	}
}
