...
CElementSpringXYZ::CElementSpringXYZ(const size_t from_node_id, const size_t to_node_id, const num_t Kx,const num_t Ky,const num_t Kz ) :
...
/** Return the stiffness submatrices between each pair of edges in this element, for the current element state.
  */
void CElementSpringXYZ::getLocalStiffnessMatrices( openbeam::aligned_containers<TStiffnessSubmatrix>::vector_t  &outSubMats ) const
{
	outSubMats.resize(3);
...
void CElementSpringXYZ::loadParamsFromSet( const TParamSet & params, const TEvaluationContext &eval)
{
...
void CElementSpringXYZ::do_mesh(const size_t my_idx, CStructureProblem &out_fem, TMeshOutputInfo &out_info, const TMeshParams & params)
{
