	...
	const bool U_b_all_zeros)
{
	...
	if ( !U_b_all_zeros )
	{
		...
		out_info.U_f -= K_decomp.solve( Kbf.transpose() * out_info.build_info.U_b );
	}
}

// ------------------------------------------------------------------------------------------
// Solve the complete FE problem, returning the resulting displacements and reaction forces.
// ------------------------------------------------------------------------------------------
void CFiniteElementProblem::solveStatic(
	TStaticSolveProblemInfo     & out_info,
	const TStaticSolverOptions  & opts
	)
{
	...
	// Iteration loop: will do only ONE iteration for linear, small-deformation problems:
	...
	while (nIter<nMaxIters)
	{
		...
		// Decomposition of a positive definite matrix for fast solving Ax=b below:
		switch (opts.algorithm)
		{
			case ssLLT:
			{
				...
				{
					// There's a structure problem! Probably it's not a structure, but a mechanism...
					throw std::runtime_error("[CFiniteElementProblem::solveStatic] Ill-conditioned matrix. Too few restrictions?");
				}
			...
			case ssSVD:
			{
				const Eigen::JacobiSVD<TDynMatrix> Kff_llt(Kff,  Eigen::ComputeThinU | Eigen::ComputeThinV);
		...
		if (opts.nonLinearIterative)
		{
			const num_t change_norm_L2 = std::sqrt( out_info.U_f.array().square().sum() / out_info.U_f.size() );
			OB_MESSAGE(1) << "[CFiniteElementProblem::solveStatic] Iter " << nIter << ", change_norm_L2 = " << change_norm_L2 << endl;

			for (size_t i=0;i<out_info.build_info.free_dof_indices.size();i++)
			{
				const num_t delta_val = out_info.U_f[i]; // The increment for this DoF
				...
				else
				{
					...
					if (dof.dof==3)      rx=delta_val;
					else if (dof.dof==4) ry=delta_val;
					else if (dof.dof==5) rz=delta_val;

					m_node_poses[ dof.node_id ].r.setRot( m_node_poses[ dof.node_id ].r.getRot() * TRotation3D(rx,ry,rz).getRot() );
				}
			}
		...
		{
			break; // Only one iteration is enough for linear problems with small displacements.
		}

	} // end iterative loops

	// If doing iterations, the U_f displacements are actually only wrt the contents of "m_node_poses":
	...
	// Next, reactions:
	if ( !U_b_all_zeros )
			out_info.F_b = out_info.build_info.K_bb*out_info.build_info.U_b + out_info.build_info.K_bf * out_info.U_f;
	else	out_info.F_b = out_info.build_info.K_bf * out_info.U_f;

	// Add the contributions of distributed forces to the reactions:

	// typedef std::map<size_t,num_t> TListOfLoads;
	// m_loads_at_each_dof_equivs: The vector of overall loads (F_L') on each DoF due to distributed forces
	//  Map keys are indices of \a m_problem_DoFs
	for (TListOfLoads::const_iterator it = m_loads_at_each_dof_equivs.begin(); it!=m_loads_at_each_dof_equivs.end();++it)
	{
		const size_t idx_dof = it->first;
		const double F_value = it->second;

		const size_t idx_restricted = out_info.build_info.dof_types[idx_dof].bounded_index;
