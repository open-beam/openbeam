...
{
	bool hasKey(const std::string &key) const { return key_vals.end()!=key_vals.find(key); }
...
// Fwd decl:
void parseParams(const vector_string &tokens, TParsedParams & params, size_t first_idx = 1);
bool replace_paramsets(TParamSet &inout_params,const std::map<std::string,TParamSet,ci_less> & user_param_sets,const TEvaluationContext &eval_context);


#define REPORT_ERROR(_MSG) \
	if (eval_context.err_msgs) { \
		eval_context.err_msgs->push_back(openbeam::format("Line %u: %s",eval_context.lin_num, std::string(_MSG).c_str())); \
		eval_context.err_msgs->push_back(openbeam::format("Line %u:  --> '%s'",eval_context.lin_num,eval_context.lin->c_str())); \
	} else \
	{ \
		std::cerr << openbeam::format("Line %u: %s",eval_context.lin_num, std::string(_MSG).c_str()); \
		std::cerr << openbeam::format("Line %u:  --> '%s'",eval_context.lin_num,eval_context.lin->c_str()); \
		return false; \
	}

#define REPORT_WARNING(_MSG) \
	if (eval_context.warn_msgs) { \
		eval_context.warn_msgs->push_back(openbeam::format("Line %u: (Warning) %s", eval_context.lin_num, std::string(_MSG).c_str() )); \
		eval_context.warn_msgs->push_back(openbeam::format("Line %u:  --> '%s'",eval_context.lin_num,eval_context.lin->c_str())); \
	} else \
	{ \
		std::cerr << openbeam::format("Line %u: (Warning) %s", eval_context.lin_num, std::string(_MSG).c_str()); \
		std::cerr << openbeam::format("Line %u:  --> '%s'",eval_context.lin_num,eval_context.lin->c_str()); \
		return false; \
	}
	...
	vector_string *warn_msgs)
{
	...
	if (!f.open(file))
	{
		if (err_msgs)  err_msgs->push_back(std::string("Error opening file for reading: ")+file);
		return false;
	}
	...
	vector_string *warn_msgs)
{
	...
	try
	{
		...
		{
			const unsigned int lin_num = static_cast<unsigned int>(f.getCurrentLineNumber());
			...
			if (strCmpI(toks[0],"VAR"))
			{
				...
				if (toks.size()!=2)
				{
					REPORT_ERROR("Expected syntax 'VAR,<NAME>=<VALUE>' [',' tokenization]");
				}
				else
				{
					vector_string parts;
			...
			else if (strCmpI(toks[0],"PARAMSET"))
			{
				...
				if (!params.hasKey("id"))
				{
					REPORT_ERROR("Missing mandatory field 'id'. Expected: 'PARAMSET,id=<ID>, <NAM1>=<VAL1> [,<NAM2>=<VAL2> [,...] ]'");
				}
				else
				{
			...
			case sGeometry:
				{
					...
					{
						// Syntax: NODE, ID=<id>, <X>, <Y> [, <Z> , [<ROT_Z>, [<ROT_Y>, [<ROT_X>] ] ] ]
						if (toks.size()<4 || toks.size()>8)
						{
							REPORT_ERROR("Expected syntax 'NODE, ID=<id>, <X>, <Y>  [, <Z> , [<ROT_Z>, [<ROT_Y>, [<ROT_X>] ] ] ]' [tokenization]");
						}
						else
						{
							...
							{
								REPORT_ERROR("Expected syntax 'NODE, ID=<id>, <X>, <Y>  [, <Z> , [<ROT_Z>, [<ROT_Y>, [<ROT_X>] ] ] ]' [problem around the ID]");
							}
							else
							{
								...
								if (EVALUATE_EXPRESSION(sIDVal,id_val))
								{
									...
									{
										REPORT_ERROR("Expected syntax 'NODE, ID=<id>, <X>, <Y>  [, <Z> , [<ROT_Z>, [<ROT_Y>, [<ROT_X>] ] ] ]' [id must be a possitive integer]");
									}
									else
									{
										...
										if (EVALUATE_EXPRESSION(toks[2],X) &&
											...
											(!has_z || EVALUATE_EXPRESSION(toks[4],Z))  &&
											(!has_rot_z || EVALUATE_EXPRESSION(toks[5],rot_Z)) &&
											(!has_rot_y || EVALUATE_EXPRESSION(toks[6],rot_Y)) &&
											(!has_rot_x || EVALUATE_EXPRESSION(toks[7],rot_X)) )
										{
											...
											// Set node pose:
											TRotationTrans3D node_pose(X,Y,Z, DEG2RAD(rot_X),DEG2RAD(rot_Y),DEG2RAD(rot_Z));
											this->setNodePose(ID, node_pose);

											needToRecomputeDOFs = true;

											OB_MESSAGE(3) << "Adding node #" << ID << " at (" << X << "," << Y << "," << Z << "," << rot_X << "," << rot_Y << "," << rot_Z << ")\n";
										}
									}
								}
							}
					...
					{
						// Syntax: ELEMENT, type=<TYPE>, from=<FROM>, to=<TO>, [,id=ID], [...]
						TParsedParams params;
						parseParams(toks,params);

						if (!params.hasKey("type") || !params.hasKey("from") || !params.hasKey("to") )
						{
							REPORT_ERROR("Missing mandatory fields: ELEMENT, type=<TYPE>, from=<FROM>, to=<TO>, [,id=ID], [...]");
						}
						else
						{
							...
							num_t nFrom=0,nTo=0;
							if (EVALUATE_EXPRESSION(params.key_vals["from"],nFrom) &&
							    EVALUATE_EXPRESSION(params.key_vals["to"],nTo) )
							{
								const size_t nNodes = this->getNumberOfNodes();
								if (nFrom<0 || nTo<0 || nFrom>=nNodes || nTo>=nNodes)
								{
									REPORT_ERROR("'from' and 'to' IDs must be valid IDs of existing nodes");
								}
								else
								{


									CElement *el = CElement::createElementByName(eType);
									if (!el)
									{
									...
									else
									{
										...
										if (REPLACE_PARAMSETS(params.key_vals))
										{
											...
											OBASSERT(el->conected_nodes_ids.size()==2)
											el->conected_nodes_ids[0] = static_cast<size_t>( nFrom );
											el->conected_nodes_ids[1] = static_cast<size_t>( nTo );
											// And process the rest of params:
											el->loadParamsFromSet(params.key_vals,eval_context);
					...
					{
						// Syntax: CONSTRAINT, node=<ID>, dof=<DOF> [, val=<VAL>]
						TParsedParams params;
						parseParams(toks,params);
						...
						if (REPLACE_PARAMSETS(params.key_vals))
						{
							...
							{
								REPORT_ERROR("Missing mandatory fields: CONSTRAINT, node=<ID>, dof=<DOF> [, val=<VAL>]");
							}
							else
							{
								const std::string & sNode = params.key_vals["node"];
								const std::string & sDof  = params.key_vals["dof"];

								num_t fNodeId;
								if (EVALUATE_EXPRESSION(sNode,fNodeId) )
								{
									if (fNodeId!=floor(fNodeId) || fNodeId<0)
									{
										REPORT_ERROR("Node ID must be a non-negative integer");
									}
									else
									{
										// Get the value of the constrained displacement (by default=0):
										num_t constr_val=0;

										if (!params.hasKey("val") ||
											EVALUATE_EXPRESSION(params.key_vals["val"],constr_val) )
										{
											const size_t nodeId = static_cast<size_t>(fNodeId);

											// Before adding constrains, we must analyze the list of DoFs in the problem:
											if (needToRecomputeDOFs)
											{
												OB_MESSAGE(4) << "Recomputing list of DoFs before introducing new constraints.\n";
											...
											for (size_t i=0;i<listDofNamesCount;i++)
											{
												if (strCmpI(listDofNames[i].name,sDof))
												{
													for (int k=0;k<6;k++)
														...
														{
															const size_t globalIdxDOF = this->getDOFIndex(nodeId , TDoFIndex(k));
															if (globalIdxDOF!=string::npos)
															{
																this->insertConstraint( globalIdxDOF, constr_val );
															}
															else
															{
																if (warn_unused_constraints)
																	REPORT_WARNING(format("Constraint ignored since the DoF %i is not considered in the problem geometry.",k))
															}
														}
													found=true;
													break;
											...
											if (!found)
											{
												REPORT_ERROR("Field 'dof=...' has an invalid value in CONSTRAINT");
											}
										}
									}
								}
			...
			case sLoads:
				{
					if (strCmpI(toks[0],"LOAD"))
					{
						...
						if (REPLACE_PARAMSETS(params.key_vals))
						{
							if (!params.hasKey("node") || !params.hasKey("dof") || params.rest_values.size()!=1 )
							{
								REPORT_ERROR("Wrong syntax. Expected is: LOAD, node=<ID>, dof=<DOF>, <VALUE>");
							}
							else
							{
								const std::string & sNode = params.key_vals["node"];
								...
								if (EVALUATE_EXPRESSION(sNode,fNodeId) )
								{
									...
									else
									{
										...
										if (EVALUATE_EXPRESSION(params.rest_values[0],load_val) )
										{
											// Before adding constrains, we must analyze the list of DoFs in the problem:
											if (needToRecomputeDOFs)
											{
												...
												if (strCmpI(listDofNames[i].name,sDof))
												{
													for (int k=0;k<6;k++)
														if (listDofNames[i].dofs[k])
														{
															...
															if (globalIdxDOF!=string::npos)
															{
																this->addLoadAtDOF(globalIdxDOF , load_val );
															}
															else
															{
																REPORT_WARNING(format("Load ignored since the DoF %i is not considered in the problem geometry.",k));
															}
														}
													found=true;
													break;
					...
					else if (strCmpI(toks[0],"ELOAD"))
					{
						...
						if (!myObj)
						{
							REPORT_ERROR("ELOAD command only applicable to openbeam::CStructureProblem objects.")
						}
						else
						{
							// Syntax: ELOAD, element=<ID>, type=<TYPE>, <P1>=<K1>, <P2>=<K2>,...
							TParsedParams params;
							parseParams(toks,params);

							if (!params.hasKey("type") || !params.hasKey("element") )
							{
								REPORT_ERROR("Missing mandatory fields: ELOAD, element=<ID>, type=<TYPE>, [...]");
							}
							else
							{
								const std::string sType = params.key_vals["type"];
								const std::string sID   = params.key_vals["element"];
								...
								if (EVALUATE_EXPRESSION(sID,fID) )
								{
									...
									{
										REPORT_ERROR("Element ID not a valid ID of existing element.");
									}
									else
									{
										const size_t eleIdx = static_cast<size_t>(fID);

										CLoadOnBeam *load = CLoadOnBeam::createLoadByName(sType);
										if (!load)
										{
										...
										else
										{
											...
											if (REPLACE_PARAMSETS(params.key_vals))
											{
												...
												load->loadParamsFromSet(params.key_vals, eval_context);
												// And add:
												myObj->addLoadAtBeam(eleIdx, load);
											}
										}
									}
								}
	...
	catch(std::exception &e)
	{
		...
		if (sErr.empty())
		{
			// If we catch an exception is because err_msgs=NULL but we found an error, which was already dumped to cerr.
			// So just return false and we're done.
			return false;
		}
		else
		{
			if (err_msgs) err_msgs->push_back(sErr);
			else  std::cerr << sErr << std::endl;
...
//  Should only process the case of returning true and do nothing else on return false to handle the error.
bool TEvaluationContext::parser_evaluate_expression(
	const std::string &sVarVal,
	num_t &val) const
{
	...
	else
	{
		...
		const TEvaluationContext &eval_context = *this; // Just a trick to make the macros REPORT_ERROR below work.

		// Handle possible errors:
		switch (ret)
		{
			case ExpressionEvaluator::eval_unbalanced:		REPORT_ERROR(format("Unbalanced expression evaluating variable value: '%s'",sVarVal.c_str())); break;
			case ExpressionEvaluator::eval_invalidoperator:	REPORT_ERROR(format("Invalid operator evaluating variable value: '%s'",sVarVal.c_str())); break;
			case ExpressionEvaluator::eval_invalidoperand:	REPORT_ERROR(format("Unknown variable or operand evaluating variable value: '%s'",sVarVal.c_str())); break;
			case ExpressionEvaluator::eval_evalerr:
			default:
				REPORT_ERROR(format("Evaluation error evaluating variable value: '%s'",sVarVal.c_str()));
				break;
		}
		if (!err_msgs)
	...
	const TEvaluationContext &eval_context)
{
	...
	if (it==inout_params.end())
		return true; // No paramset, go on.

	std::map<std::string,TParamSet,ci_less>::const_iterator itPar = user_param_sets.find(it->second);
