    ...
    const TStaticSolveProblemInfo & solver_info )
{
	...
	// For convenience, firstly build a simple list with the global displacement of all DOFs,
	...
	for (size_t i=0;i<nE;i++)
	{
		...
		for (size_t f=0;f<nFaces;f++)
		{
			...
			Uel[f].block(0,0,3,1).noalias() = el->getGlobalOrientation().getRot().transpose() * Uf.block(0,0,3,1);
...
#else
			OB_TODO("This only works for planar (Z=0) structures!!! Debug formulas above")
			Uel[f][3] = Uf[3];
			Uel[f][4] = 0;
			Uel[f][5] = Uf[5];
#endif
	...
	// Change local coordinates to "strength of materials", with our sign conventions:
	//
	for (size_t i=0;i<nE;i++)
	{
		TElementStress &es = out_stress.element_stress[i];
	...
	// Don't forget to add those stress values in "m_extra_stress_for_each_element;
	for (std::map<size_t, TElementStress>::const_iterator it=m_extra_stress_for_each_element.begin();it!=m_extra_stress_for_each_element.end();++it)
	{
