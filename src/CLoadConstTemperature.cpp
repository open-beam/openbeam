...
void CLoadConstTemperature::loadParamsFromSet( const TParamSet & params,const TEvaluationContext &eval)
{
...
void CLoadConstTemperature::meshLoad(CStructureProblem &meshed_fem,const std::vector<size_t> & meshed_element_idxs, const size_t original_bar_idx, const CStructureProblem & original_fem) const
{
	// Temperature loads are just decomposed into identical loads at each element:
	for (size_t i=0;i<meshed_element_idxs.size();i++)
		meshed_fem.addLoadAtBeam( meshed_element_idxs[i],  new CLoadConstTemperature( this->incr_temp ));
}
