...
CElementSpring::CElementSpring(const size_t from_node_id, const size_t to_node_id, const num_t K ) :
...
/** Return the stiffness submatrices between each pair of edges in this element, for the current element state.
  */
void CElementSpring::getLocalStiffnessMatrices( openbeam::aligned_containers<TStiffnessSubmatrix>::vector_t  &outSubMats ) const
{
	outSubMats.resize(3);
	...
	if (K==UNINITIALIZED_VALUE)
		throw std::runtime_error(format("CElementSpring: Uninitialized parameter K (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
...
void CElementSpring::loadParamsFromSet( const TParamSet & params, const TEvaluationContext &eval)
{
...
void CElementSpring::drawQtSVG(QSvgGenerator &svg, const TDrawStructureOptions &options, const TRenderInitData & ri, const TDrawElementExtraParams &draw_el_params, const TMeshOutputInfo *meshing_info) const
{
}
#endif
...
void CElementSpring::drawSVG(void *_cairo_context,const TDrawStructureOptions &options,  const TRenderInitData & ri,const TDrawElementExtraParams &draw_el_params,const TMeshOutputInfo *meshing_info) const
{
	...
	obj_pose.t = p0.t;
	internal::computeOrientationFromTwoPoints(p0.t,p1.t,0 /* rotation on local X */,obj_pose.r);

	// Draw:
	cr->set_source_rgba(0.4,0.4,0.4, draw_el_params.color_alpha );
	cr->set_line_width( EDGE_WIDTH );
	internal::drawLocalScaledSegments(cr,obj_pose, p1_p0_norm /* scale */, local_pts );
...
void CElementSpring::do_mesh(const size_t my_idx, CStructureProblem &out_fem, TMeshOutputInfo &out_info, const TMeshParams & params)
{
