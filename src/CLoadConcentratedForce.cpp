	...
	)
{
	...
	if (std::abs(proy_long)>1e-5) throw std::runtime_error("Only perpendicular forces supported by now!");

	const num_t a = this->dist;
	const num_t b = L-a;

	if (a<0 || b<0) throw std::runtime_error("Error: Point of application is out of the beam!");
...
void CLoadConcentratedForce::loadParamsFromSet( const TParamSet & params,const TEvaluationContext &eval)
{
...
void CLoadConcentratedForce::meshLoad(CStructureProblem &meshed_fem,const std::vector<size_t> & meshed_element_idxs, const size_t original_bar_idx, const CStructureProblem & original_fem) const
{

	// Find out the original length of the bar:
	const size_t node0 = meshed_fem.getElement( *meshed_element_idxs.begin() )->conected_nodes_ids[0];
	const size_t node1 = meshed_fem.getElement( *meshed_element_idxs.rbegin() )->conected_nodes_ids[1];
	const double org_bar_length = meshed_fem.getNodePose(node0).distanceTo( meshed_fem.getNodePose(node1) );

	const size_t nE = meshed_element_idxs.size();
	double Al = org_bar_length / nE;

	// Create the new load:
	const size_t idx_affected_element = std::min( floor( this->dist / Al ) , static_cast<double>(nE-1));
