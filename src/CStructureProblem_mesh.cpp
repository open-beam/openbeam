	...
	)
{
	...
	// ------------------------------
	// (4) Constraints:
	// Copy from the original fem, since they can only appear in the original nodes:
	// ------------------------------
	...
	this->updateListDoFs(); 
	for (TListOfConstraints::const_iterator itC = this->m_DoF_constraints.begin();itC != this->m_DoF_constraints.end();++itC)
	{
		...
		const size_t globalIdxDOF = out_fem.getDOFIndex(original_node_id , TDoFIndex( dof.dof ) /* 0:dx,1:dy,... */ );
	...
	// (4) LOADS: at elements
	// ------------------------------
	for (std::multimap<size_t,CLoadOnBeam*>::const_iterator itLoad = m_loads_on_beams.begin();itLoad != m_loads_on_beams.end();++itLoad)
	{
		const size_t org_bar_idx = itLoad->first;
		const CLoadOnBeam * load = itLoad->second;

		load->meshLoad(out_fem, out_info.element2elements[org_bar_idx], org_bar_idx, *this);
	}
