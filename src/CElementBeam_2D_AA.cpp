...
/** Return the stiffness submatrices between each pair of edges in this element, for the current element state.
  */
void CElementBeam_2D_AA::getLocalStiffnessMatrices( openbeam::aligned_containers<TStiffnessSubmatrix>::vector_t &outSubMats ) const
{
	outSubMats.resize(3);
	...
	if (A==UNINITIALIZED_VALUE) throw std::runtime_error(format("CElementBeam_2D_AA: Uninitialized parameter A (Element in %u->%u)", static_cast<unsigned int>(this->conected_nodes_ids[0]), static_cast<unsigned int>(this->conected_nodes_ids[1]) ));
