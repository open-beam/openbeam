...
namespace openbeam
{
	struct TRenderInitData; //Fwd. decl. (defined in CFiniteElementProblem::saveAsImage)

	struct TDoF
	{
		inline TDoF(size_t _node_id, unsigned char _dof)  : node_id(_node_id), dof(_dof) { }
	...
	{
		struct TDoFType {
			size_t bounded_index; //!< Index within F_b or U_b, if the DOF is bounded; string::npos otherwise
			size_t free_index;    //!< Index within F_f or U_f, if the DOF is free; string::npos otherwise
		};
		...
		Eigen::Matrix<num_t,Eigen::Dynamic,1>  U_b;  //!< A vector of length = "bounded_dof_indices.size()" with all the constrains, each value for one DoF, in the order as they appear in \a bounded_dof_indices
		Eigen::Matrix<num_t,Eigen::Dynamic,1>  F_f;  //!< A vector of length = "free_dof_indices.size()" with the overall load at each free DOF, in the order as they appear in \a free_dof_indices
	};

	/** Output information from \a solveStatic() */
	struct TStaticSolveProblemInfo
	{
		TBuildProblemInfo                       build_info;      //!< Information from the assembly of the problem
		Eigen::Matrix<num_t,Eigen::Dynamic,1>   F_b;  //!< A vector of length = "build_info.bounded_dof_indices.size()" with all the reaction forces, DOFs in the same order than \a bounded_dof_indices
		Eigen::Matrix<num_t,Eigen::Dynamic,1>   U_f;  //!< A vector of length = "build_info.free_dof_indices.size()" with the displacement of free DOFs, in the same order than \a free_dof_indices

		Eigen::Matrix<num_t,Eigen::Dynamic,1>   F;  //!< The full F vector (bounded+free DOFs)
		Eigen::Matrix<num_t,Eigen::Dynamic,1>   U;  //!< The full U vector (bounded+free DOFs)
	};
	...
	struct TMeshOutputInfo
	{
		...
		size_t num_original_nodes; //!< The first N nodes in the FEM correspond to the original nodes in the structure before meshing. This variable holds that "N".
		std::deque<std::vector<size_t> >  element2nodes;    //!< List of all intermediary nodes (incl the original ones) at which element [i] is connected.
	...
	/** A complete problem of finite elements.
	  ...
	  *     - Nodes: A list of 6 DoF nodes at certain space locations. Elements are always defined between 2 or more nodes.
	  *     - Constraints: External loads and DoF movement restrictions. They are applied over the nodes.
	  *
	...
	public:
        ...
        virtual void clear(); //!< Delete all elements, nodes and constraints in this problem, completely emptying it.

		/** Discard the current problem and loads it by parsing the given text input stream.
		  ...
		  */
		bool loadFromStream(std::istream &is, vector_string *errMsg=NULL,vector_string *warnMsg=NULL);
		...
		/** Discard the current problem and loads it by parsing the given text file.
		  ...
		  *  \return true on Success
		  */
		bool loadFromFile(const std::string &file, vector_string *errMsg=NULL,vector_string *warnMsg=NULL);
        ...
        /** Insert a new element in the problem (this objects owns the memory, will delete the element object when not needed anymore).
		...
		/** Adds a new displacement constraint to the problem, at the DoF \a dof_index (indices wrt list returned by \a getProblemDoFs() ).
          */
        void insertConstraint( const size_t dof_index, const num_t value = 0 );

		const TListOfConstraints & getAllConstraints() const { return m_DoF_constraints; }
		...
		const TListOfLoads & getOverallLoadsOnDOFs() const { return m_loads_at_each_dof; }


		/** @} */
		// ----------------------------------------------------------------------------
		/** @name Problem nodes and their reference 3D pose (to this, one must add the displacement obtained as solution to the problem).
		    @{ */
		...
		/** Insert a new node to the problem with the given 6D pose, in global coordinates. \return The index of this new node */
        ...
        /** The 3D location of a node; its orientation (if different than global coordinates) serves as a local frame for constraints. */
		      TRotationTrans3D & getNodePose(size_t i)       { OBASSERT_DEBUG(i<m_node_poses.size()); return m_node_poses[i]; }
        /** The 3D location of a node; its orientation (if different than global coordinates) serves as a local frame for constraints. */
		const TRotationTrans3D & getNodePose(size_t i) const { OBASSERT_DEBUG(i<m_node_poses.size()); return m_node_poses[i]; }
		...
		/** @} */
		// ----------------------------------------------------------------------------
		/** @name Problem DOFs (abstract list of DoF, doesn't have quantitative displacement values)
		    @{ */

		/** Return the list of all DoF in the problem . \sa getProblemDoFsDescription */
		const std::vector<TDoF> & getProblemDoFs() { updateListDoFs(); return m_problem_DoFs; }

		/** Make a list with all DoF in the problem sorted by nodes. \sa getProblemDoFs */
		...
		{
			TStaticSolverOptions() :
				algorithm           (ssLLT),
				nonLinearIterative  (false)
			{ }
		...
		struct TStressInfo
		{
		    ...
		    /** The i'th entry is another vector with the stress of all the faces of the i'th element.
		      *  Typically for 1D elements (beams, springs) each entry will only contain two stress values, one for each end of the element.
		      */
		    std::vector<TElementStress>  element_stress;
		};


		/** For any partitioning of the DoFs into fixed (restricted, boundary conditions)
		  *  and the rest (the free variables \a free_dof_indices), where DoF indices refer to \a m_problem_DoFs,
		  *  computes three sparse matrices: K_{FF} (free-free), K_{BB} (boundary-boundary) and
		  *  K_{BF} (boundary-free) which define the stiffness matrix of the problem.
		  *  \note The free vs constrained DoF's are automatically determined from \a m_DoF_constraints
		  * \sa solveStatic
		  */
		...
		/** Solve the static FE problem, returning the resulting displacements and reaction forces.
		  ...
		  */
		void solveStatic( TStaticSolveProblemInfo & out_info, const TStaticSolverOptions &opts = TStaticSolverOptions() );


		/** After solving the problem with \a solveStatic, you can optionally call this post-processing method to evaluate the stress of all the elements.
		  *
		  * \sa solveStatic
		  */
	...
	protected:
		...
		/** List of constrainsts for each "fixed/constrained" DoF.
		  ...
		  *  Map values are displacement in that DoF wrt the current node pose. Units are SI (meters or radians).
		  *  \sa getProblemDoFs
		  */
		TListOfConstraints  m_DoF_constraints;

		/**  The vector of overall external loads (F_L) - Map keys are indices of \a m_problem_DoFs  */
		TListOfLoads        m_loads_at_each_dof;

		/**  The vector of overall loads (F_L') on each DoF due to distributed forces - Map keys are indices of \a m_problem_DoFs
		  *  Updated by \a assembleProblem() via \a internalComputeStressAndEquivalentLoads()
		  */
		TListOfLoads        m_loads_at_each_dof_equivs;

		/**  The extra stress to add up to each element as computed by \a internalComputeStressAndEquivalentLoads
        ...
        bool internal_loadFromFile(void *f,vector_string *err_msgs,vector_string *warn_msgs);
		...
		{
			array<int,6> dof_index;  //!< Index of that DOF into \a m_problem_DoFs, or -1 if DOF not considered in the problem.
		...
		/** Built together with m_problem_DoFs by \a updateListDoFs , holds the mapping from node index -> 6 numbers, with the index
		  *  of that global DOF in the list of problem DOFs, or -1 if the DOF is not considered.
		  */
        ...
        typedef std::map<size_t,TNodeElementConnection> TNodeConnections; //!< For each node, this map has the list of connected elements and the type of the connection.
