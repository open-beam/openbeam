...
namespace openbeam
{
	...
	struct TDrawStructureOptions
	{
		...
		TDrawStructureOptions() :
			// --- bool switches ----
			show_nodes_original(true),show_nodes_deformed(false),show_node_labels(false),
			show_elements_original(true),show_elements_deformed(false),show_element_labels(true),
			show_loads(true),
			...
			constraints_original_alpha(1), constraints_deformed_alpha(1),
			loads_original_alpha(1), loads_deformed_alpha(1)
		{
		}
	...
	struct TDrawElementExtraParams
	{
		...
		bool   draw_original_position; //!< If false, draw the final deformed position.
