...
/** Usage: OB_MESSAGE(num) << "blah" << x << endl;
  *  Where num is the minimum OpenBeam verbose level for the message to actually be emitted to std::cout.
  * \sa openbeam::setVerbosityLevel
  */
#define OB_MESSAGE(VERBOSE_LEVEL) if (openbeam::getVerbosityLevel()>=VERBOSE_LEVEL) std::cout << "[" << _CURRENT_FUNC_ << "] "

// Define a decl. modifier for printf-like format checks at compile time:
#ifdef __GNUC__
#	define OB_printf_format_check(_FMT_,_VARARGS_)  __attribute__ ((__format__ (__printf__, _FMT_,_VARARGS_)))
#else
...
namespace openbeam
{
	...
	typedef std::map<std::string,std::string,ci_less>          TParamSet; //!< A set of parameters, indexed by name (case insensitive)
    ...
    struct TFaceStress
    {
        ...
        {
        	N+=o.N; Vy+=o.Vy; Vz+=o.Vz;
        	Mx+=o.Mx; My+=o.My; Mz+=o.Mz;
        	return *this;
        }
        ...
        TFaceStress & operator +=(const Eigen::MatrixBase<MAT> & o)
        {
        	...
        	N+=o[0]; Vy+=o[1]; Vz+=o[2];
        	Mx+=o[3]; My+=o[4]; Mz+=o[5];
        	return *this;
        }

    };

    typedef std::vector<TFaceStress> TElementStress; //!< One entry per element "face".

	struct TEvaluationContext
	{
		TEvaluationContext() : err_msgs(NULL),warn_msgs(NULL),lin_num(0), lin(NULL) { }
		...
		bool parser_evaluate_expression(const std::string &sVarVal,num_t &val) const;
	...
	struct TRotation3D
	{
		...
		inline void              setRot( const TMatrix33 & r) { rot=r; m_is_pure_identity=false; }

		static void matrix2angles(const TMatrix33 &R, num_t &ang_x,num_t &ang_y,num_t &ang_z);
	...
	struct TPoint3D
	{
		...
		TPoint3D() { }
		TPoint3D(const num_t x,const num_t y,const num_t z) { coords[0]=x; coords[1]=y; coords[2]=z; }
		inline num_t norm_squared() const { return coords[0]*coords[0] + coords[1]*coords[1] + coords[2]*coords[2]; }
	...
	struct TRotationTrans3D
	{
		...
		inline TRotationTrans3D(num_t x,num_t y,num_t z,num_t ang_x,num_t ang_y,num_t ang_z) : t(x,y,z),r(ang_x,ang_y,ang_z) { }

	    TPoint3D     t;  //!< Translation vector in 3D
	    TRotation3D  r;  //!< Rotation in 3D

		inline num_t distanceTo(const TRotationTrans3D &o) const { return std::sqrt( openbeam::square(t.coords[0]-o.t.coords[0])+openbeam::square(t.coords[1]-o.t.coords[1])+openbeam::square(t.coords[2]-o.t.coords[2]) ); }
    ...
    /** Changes the overall library verbosity level: 0=quiet, 1=interesting messages, 2=verbose.
	...
	void tokenize(
		...
		vector_string		&outTokens,
		bool   do_trim_all_parts = true );
...
// partial specialization for arrays of size 0
	...
	/** By creating an object of this class, all the output to std::cout (and std::cerr) will be redirected to a text file, and optionally also shown on the console.
	  *  Class copied from MRPT (http://www.mrpt.org/)
	  */
	...
	public:
		...
		CConsoleRedirector(
			...
			int bufferSize = 1000 ) : m_of(), sbOld(NULL),sbOld_cerr(NULL),m_also_to_console(also_to_console)
		{
			// Open the file:
			...
			if (!m_of.is_open()) throw std::runtime_error(format("Error opening file: %s",out_file.c_str()));
