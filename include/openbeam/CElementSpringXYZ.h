...
namespace openbeam
{
	/** A spring element with three elastic components in the local X, Y and Z directions between two given nodes.
	  */
	class CElementSpringXYZ : public CElement
	{
	public:
		CElementSpringXYZ();
		CElementSpringXYZ(const size_t from_node_id, const size_t to_node_id, const num_t Kx, const num_t Ky, const num_t Kz);
